...
class DockerRuntimeDriver:
    ...
    def __init__(self, image: str = "agentsupport-runner:dev") -> None:
        self.image = image
...
class DockerCliRuntimeDriver:
    ...
    def _container_name(session_id: UUID, lease_epoch: int) -> str:
        return f"agentsupport-runner-{session_id.hex[:12]}-{lease_epoch}"
...
def default_session_container_env() -> dict[str, str]:
    ...
    environment.setdefault("SESSION_RUNNER_TRAE_CONFIG", "/app/src/session_runner/trae_config.yaml")
