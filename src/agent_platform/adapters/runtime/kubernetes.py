...
class KubernetesRuntimeDriver:
        ...
        return {
            ...
            "metadata": {
                ...
                "labels": {
                    ...
                    "agent-platform/workspace-id": str(workspace_id),
                },
            },
            "spec": spec,
        }
    ...
    def _service_manifest(self, name: str, session_id: UUID) -> dict[str, Any]:
        return {
            ...
            "metadata": {
                "name": name,
                "labels": {"app.kubernetes.io/managed-by": "agent-platform"},
            },
            "spec": {
                "clusterIP": "None",
                "selector": {
                    "agent-platform/runtime-id": name,
                    "agent-platform/session-id": str(session_id),
                },
    ...
    def _pod_manifest(
        ...
        return {
            ...
            "metadata": {
                "name": name,
                "labels": {
                    "app.kubernetes.io/name": "agent-session-runner",
                    "app.kubernetes.io/managed-by": "agent-platform",
                    "agent-platform/runtime-id": name,
                    "agent-platform/session-id": str(session_id),
                    "agent-platform/workspace-id": str(workspace_id),
                    "agent-platform/lease-epoch": str(lease_epoch),
                },
            },
            "spec": {
                "restartPolicy": "Never",
    ...
    async def list_managed(self) -> list[dict[str, Any]]:
        async with self._client() as client:
            response = await client.get(
                ...
                params={"labelSelector": "app.kubernetes.io/managed-by=agent-platform"},
            )
        response.raise_for_status()
