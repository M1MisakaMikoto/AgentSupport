...
def service(tmp_path):
    return PlatformService(Settings(workspace_root=tmp_path))
...
@pytest.mark.asyncio
async def test_platform_checkpoint_preserves_pending_tool_batch(service):
    workspace = service.create_workspace("demo")
    session = service.create_session(workspace.id)
...
async def test_workspace_write_lease_serializes_sessions(tmp_path):
    service = PlatformService(Settings(workspace_root=tmp_path, max_active_sessions=2))
...
async def test_container_start_failure_is_recorded_as_structured_run_failure(tmp_path):
    ...
    service = PlatformService(Settings(workspace_root=tmp_path))
...
@pytest.mark.asyncio
async def test_platform_dispatches_to_runner_and_releases_completed_session(service):
...
@pytest.mark.asyncio
async def test_platform_maps_persisted_workspace_path_into_shared_runner(tmp_path):
    service = PlatformService(
        Settings(
...
@pytest.mark.asyncio
async def test_platform_keeps_session_while_runner_waits_for_input(service):
    service.core_runtime = TraeCoreRunnerRuntime(
        "http://runner", transport=ASGITransport(app=create_runner_app())
    )
...
@pytest.mark.asyncio
async def test_platform_forwards_approval_to_runner(service):
    service.core_runtime = TraeCoreRunnerRuntime(
        "http://runner", transport=ASGITransport(app=create_runner_app())
    )
...
@pytest.mark.asyncio
async def test_platform_forwards_cancel_to_runner(service):
    service.core_runtime = TraeCoreRunnerRuntime(
        "http://runner", transport=ASGITransport(app=create_runner_app())
    )
...
@pytest.mark.asyncio
async def test_paused_runner_resumes_from_platform_checkpoint(service):
    service.core_runtime = TraeCoreRunnerRuntime(
        "http://runner", transport=ASGITransport(app=create_runner_app())
    )
...
@pytest.mark.asyncio
async def test_platform_checkpoint_uses_container_workspace_reference(service):
...
async def test_health_supervisor_requires_repeated_failures(tmp_path):
    ...
    service = PlatformService(Settings(workspace_root=tmp_path, health_failure_threshold=2))
...
async def test_health_supervisor_releases_orphan_container_lease(tmp_path):
    service = PlatformService(Settings(workspace_root=tmp_path, health_failure_threshold=1))
