...
@pytest.fixture
def postgres_docker_database_url():
    base_url = make_url(
        os.getenv(
...
@pytest.mark.asyncio
async def test_real_postgres_restart_recovers_active_session(
    tmp_path: Path, postgres_docker_database_url: str
):
    config = {
        "database_url": postgres_docker_database_url,
