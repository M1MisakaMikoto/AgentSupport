...
from agent_platform.worker import DistributedWorker
...
async def test_waiting_run_accepts_command_from_another_api_instance(tmp_path):
    config = _settings(tmp_path)
    first_api = PlatformService(config)
    second_api = PlatformService(config)
