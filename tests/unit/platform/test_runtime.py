...
from agent_platform.runtime import DockerCliRuntimeDriver, default_session_container_env
...
async def test_docker_cli_driver_enforces_session_security_and_endpoint(tmp_path: Path):
    ...
    skill.mkdir()
    driver = FakeDockerCliRuntimeDriver("agent-session-session-1")
...
async def test_docker_cli_driver_removes_container_that_exited_before_stop():
    ...
    driver = AlreadyExitedDriver("agent-session-exited")
...
async def test_docker_cli_driver_escalates_sigterm_to_sigkill_after_grace_period():
    ...
    driver = GracefulTimeoutDriver("agent-session-timeout")
    driver.running = True
