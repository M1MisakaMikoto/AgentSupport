...
class _BashSession:
    ...
    def __init__(self) -> None:
        ...
        self._encoding = locale.getpreferredencoding(False) if os.name == "nt" else "utf-8"
    ...
    async def start(self) -> None:
        ...
        if os.name != "nt":  # Unix-like systems
            ...
            self._process = await asyncio.create_subprocess_shell(
                "cmd.exe /d /q /v:on",  # disable AutoRun/echo; enable `!errorlevel!`
                shell=True,
                bufsize=0,
                stdin=asyncio.subprocess.PIPE,
                stdout=asyncio.subprocess.PIPE,
    ...
    async def run(self, command: str) -> ToolExecResult:
        ...
        sentinel = self._sentinel.replace("__ERROR_CODE__", errcode_retriever)
        if os.name == "nt":
            command_payload = f"({command}){command_sep} echo {sentinel}\r\n"
        else:
            command_payload = f"(\n{command}\n){command_sep} echo {sentinel}\n"
        self._process.stdin.write(command_payload.encode(self._encoding))
        ...
        try:
            async with asyncio.timeout(self._timeout):
                while True:
                    ...
                    output: str = self._process.stdout._buffer.decode(self._encoding)  # type: ignore[attr-defined] # pyright: ignore[reportAttributeAccessIssue, reportUnknownMemberType, reportUnknownVariableType]
        ...
        error: str = self._process.stderr._buffer.decode(self._encoding)  # type: ignore[attr-defined] # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType, reportAttributeAccessIssue]
        if error.endswith("\n"):  # pyright: ignore[reportUnknownMemberType]
...
class BashTool(Tool):
    ...
    def get_description(self) -> str:
        if os.name == "nt":
            return """Run commands in a persistent Windows cmd.exe shell
* Use Windows cmd.exe syntax and built-in commands; do not use Bash-only syntax.
