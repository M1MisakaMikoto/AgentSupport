...
当前开发者控制台不能完成 API 全量测试。

| 入口 | 当前能力 | API 覆盖 |
| --- | --- | --- |
| 部署控制页 `/` | 部署、扩缩容、状态、静态检查、pytest、在线冒烟 | 自动验收直接请求 7/13 个公共路径 |
| 任务调试页 `/tasks/` | 创建资源、执行任务、输入、审批、取消、查询和订阅事件 | 可手工触发 9/13 个公共路径 |
| 两个入口合计 | 可以验证主任务链和部分运维能力 | 请求过 11/13 个路径，但没有逐项断言完整契约 |

两个入口都没有覆盖 `GET /cores` 和 `GET /sessions/{session_id}/events`。任务调试页还不覆盖
`GET /ready` 和 `GET /metrics`。更重要的是，页面操作没有系统验证请求校验、错误响应、幂等冲突、
乐观并发、SSE 断线续传、容量限制和依赖故障，因此不能把“页面调用成功”等同于“API 全量通过”。

控制台中的“回归验收”适合作为开发入口，但最终结论必须来自可重复、带断言和报告的自动化测试。
