...
| 部署 · API 契约验收 | 结构化断言运维、资源、事件、交互四类用例，含错误路径、幂等、乐观并发、SSE 断线续传；按 OpenAPI 输出操作覆盖率 | 42 个公共路径登记；skills / mcp-servers / eval 由仓库契约套件覆盖 |
...
2. 42 个 AgentSupport 公共路径都有正常、校验失败和相关业务错误断言。
...
- OpenAPI operation 覆盖率为 100%，公共 42/42、Runner 9/9。
