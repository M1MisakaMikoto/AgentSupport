...
> **已废弃（v0.2）**：本项目自 `0.2.0` 起只管理执行资源，组织 / 用户 / 项目 / 预设的
> 业务实体交由上游管理，平台以 `tenant_id` / `user_id` / `project_id` 标签透传。
> 本设计文档仅作历史参考，相关 API 与数据表已移除，详见
> [ADR-002](../adr/002-platform-boundary-and-api-v2.md) 与
> [上游迁移指南](../migration/v0.1-to-v0.2.md)。
